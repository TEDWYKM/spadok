...
function paintOverlays() {
  ...
  if (V.sheet) {
    ...
    h += '<div class="scrim' + (s.center ? ' center' : '') + '" data-act="scrim">' +
      ...
      /* Список прокручується, а заголовок і кнопка закриття — ні.
         Доки пунктів було три, шторка просто росла вниз; на довшому
         списку вона виросла б за межі екрана, і закрити її стало б
         нічим. Прокрутка не ланцюжиться на сторінку під шторкою
         (overscroll-behavior), інакше карта під нею їде разом зі
         списком і людина губить те, що вибирала. */
      '<div class="sopts">' +
      ...
      (s.note ? '<p class="meta" style="margin:12px 2px 2px;line-height:1.5">' +
        esc(s.note) + '</p>' : '') +
      '</div>' +
...
function pickRegion() {
  /* Порожні регіони поки не показуємо. Тринадцять пунктів, кожен
     із яких відкриває порожню карту, перетворювали вибір на прокрутку
     повз них до двох потрібних — і найчастіша дія коштувала найдорожче.

     Але зникнути зовсім вони не мають права: рядком під списком
     сказано, скільки регіонів іще чекає на контент. Щойно в регіоні
     зʼявиться перша точка, він стане в перелік сам — FILLED() читає
     дані, а не окремий список «показувати».                        */
  const shown = FILLED();
  /* Якщо людина вже сидить у порожньому регіоні — з попередньої
     версії або з відновленого вибору, — не ховаємо від неї те, де
     вона зараз: інакше в списку не буде позначки «показується»,
     і вибір виглядатиме зламаним. */
  if (!isAll() && REGIONS[REG()] && shown.indexOf(REG()) < 0) shown.push(REG());
  const hidden = Object.keys(REGIONS).length - shown.length;
  const keys = [ALL].concat(shown);
  sheet({
    ...
    note: hidden
      ? 'Показані регіони, у яких уже є точки. Ще ' + hidden + ' ' +
        plural(hidden, 'регіон', 'регіони', 'регіонів') + ' із ' +
        Object.keys(REGIONS).length + ' чекають на контент і зʼявляться тут разом із ним.'
      : '',
    options: keys.map(k => {
      const here = k === REG();
      if (k === ALL) return {
