...
/* Перелік регіонів показує тільки наповнені: тринадцять пунктів,
   кожен із яких відкриває порожню карту, робили найчастішу дію
   найдорожчою. Але список, який мовчки викинув дві третини країни,
   бреше виглядом — тому поруч мусить стояти, скільки сховано. */
const pickSrc = (appCode.match(/function pickRegion\(\)[\s\S]*?\n\}/) || [''])[0];
if (!pickSrc) bad('немає pickRegion()');
else if (/\[ALL\]\.concat\(Object\.keys\(REGIONS\)\)/.test(pickSrc))
  bad('у переліку знову всі регіони, зокрема порожні — вибір перетворюється на прокрутку повз них');
else if (!/FILLED\(\)/.test(pickSrc))
  bad('перелік регіонів не спирається на FILLED() — новий наповнений регіон доведеться додавати руками');
else if (!/hidden/.test(pickSrc))
  bad('сховані регіони ніде не пораховані — список мовчки вдає, що країни менше');
else if (!/shown\.indexOf\(REG\(\)\) < 0/.test(pickSrc))
  bad('поточний регіон може випасти з переліку — зникне позначка «показується»');
else ok('у переліку лише наповнені регіони, і сказано, скільки ще чекає на контент');

/* Шторка мусить прокручуватись, а не рости за край екрана: інакше
   кнопка закриття їде за нижню межу, і шторка стає пасткою. */
if (!/class="sopts"/.test(appCode))
  bad('список у шторці не винесений в окремий блок — прокручуватись нема чому');
else if (!/\.sopts\{[^}]*overflow-y:auto/.test(css.replace(/\s*\n\s*/g, '')))
  bad('.sopts без прокрутки — довгий список винесе кнопку закриття за екран');
else if (!/overscroll-behavior:contain/.test(css))
  bad('прокрутка шторки ланцюжиться на карту під нею — людина губить те, що вибирала');
else if (!/\.sheet\{[^}]*max-height/.test(css.replace(/\s*\n\s*/g, '')))
  bad('у шторки немає стелі висоти — прокрутці нема від чого відштовхнутись');
else ok('шторка прокручується всередині себе, не тягнучи за собою карту');
